...
class SistemaController extends BaseController {
  ...
  public function manutenzioneDebug(Request $request, TranslatorInterface $trans,
    ...
    // legge .env
    $envPath = $this->getRuntimeEnvPath();
  ...
  /**
   * Restituisce il file env runtime da usare per le configurazioni modificate dal pannello.
   */
  private function getRuntimeEnvPath(): string {
    $envLocalPath = $this->getParameter('kernel.project_dir').'/.env.local';
    return file_exists($envLocalPath) ? $envLocalPath : $this->getParameter('kernel.project_dir').'/.env';
  }
